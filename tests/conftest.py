...
def _event_loop(thread_name: str):
    ...
    def run():
        ...
        try:
            log.debug("Starting event loop in thread: %s", thread_name)
            loop.run_forever()
        finally:
            log.debug("Running shutdown_asyncgens in thread: %s", thread_name)
            loop.run_until_complete(loop.shutdown_asyncgens())
            log.debug("Closing event loop in thread: %s", thread_name)
            loop.close()
            log.debug("Event loop in thread %s has been closed", thread_name)

    thread = Thread(name=thread_name, target=run, daemon=True)
    log.debug("Starting thread for event loop: %s", thread_name)
    thread.start()
    log.debug("Waiting for event loop to be ready in thread: %s", thread_name)
